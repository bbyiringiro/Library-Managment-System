...
if (isset($_GET['stud'])&& !empty($_GET['stud'])){
    ...
    if(!isset($_SESSION['out']) || $_SESSION['out']!=$_GET['stud'])
                        header("location:./");
	$stud=$_GET['stud'];
...
//sql of pending for teachers and outsiders
$sql="select b.book_title  b_name ,b.book_no 'nbr',a.due_date 'date_borrowed',a.return_date 'return' ,a.current_class 'current' from activity a inner join books b on a.book_no=b.book_no where a.s_id=? and a.fine=0 and a.outsider=1 order by date_borrowed desc";
//sql about the the history for teachers and outsiders
$sql2="select b.book_title  b_name ,b.book_no 'nbr',a.due_date 'date_borrowed',a.return_date 'return' ,a.current_class 'current' from activity a inner join books b on a.book_no=b.book_no where a.s_id=? and a.fine=1 and a.outsider=1 order by date_borrowed desc ";
 /* codes about fetching transaction from server --> */
//pending
         $pending=$db->getRows($sql,array($condition));
         //history
         $hist=$db->getRows($sql2,array($condition));
}//end of first if 
else header("location:home.php");


//checking if picture of user exist to set a defualt
if(file_exists('pictures/'.$res[0]['pro_id'].'.jpg')){
	$studentpic='pictures/'.$res[0]['pro_id'].'.jpg';
}
else 
        ...
        <div class="col m10">
          <div class="card">
            <div class="card-image">
              ...
              <span class="card-title"><?php $n=explode(' ',$res[0]['name']);echo strtoupper($n[0]);?></span>
            </div>
    ...
    <div id="tab2" class="col s12">
        ...
        <thead>
          <tr>
              ...
              <th data-field="price">ISBN</th>
              <th data-field="due_date">Due date</th>
              <th data-field="day">Day<br>returned
        	...
        	if (keyword.length >= 1) {
        		...
        		$.get( "ajax/lend.php", { book: keyword } ).done(function( data ) {
                    ...
                    if($.isEmptyObject(results)){
                        $('.book-suggest').html('<div class="not-found blue-grey-text white row valign-wrapper"><div class="col s2"><i class="mdi-action-info medium"> </i></div><div class="col s8">That book is not registered,tell  the librarian to register it </div><div class="col s2"></div></div> '); $('.modal-trigger').leanModal();
                    }
                    else{
...
//update books pending book
    function updateP_nbr(){
        ...
        $.ajax({
        	    	 url: 'ajax/lend.php',
        	    	 type:'GET',
        	    	 data:{u2:<?php echo json_encode($stud)?>,out:out},
                     beforeSend:function(){},
