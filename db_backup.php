...
define("OUTPUT_DIR","D:/Patrick/xampp_library/htdocs/library/backups"); // Folder / Directory Name
define("TABLES", '*');

/* Instantiate Backup_Database and perform backup */
if(isset($_GET['backup_db'])){
$backupDatabase = new Backup_Database(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
$backupDatabase->backupTables(TABLES, OUTPUT_DIR);
/* The Backup_Database class */
}
class Backup_Database {
/* Host where database is located  */ var $host = 'localhost';
  var $username = 'root';
  var $passwd = '';
  var $dbName = 'library';
  var $charset = '';
  ...
  protected function saveFile(&$sql, $outputDir = '.') {
    ...
    try {
      $handle = fopen($outputDir . '/backup-' . $this->dbName . '-' . date("Y-m-d-h", time()) . '.sql', 'w+');
      fwrite($handle, $sql);
      fclose($handle);
    } catch (Exception $e) {
      var_dump($e->getMessage());
